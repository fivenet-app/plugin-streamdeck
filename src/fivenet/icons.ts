...
const ICON_PATH_CACHE: Record<string, string | undefined> = {};

export function mdiIconPath(name: string): string | undefined {
	if (!name) return undefined;
	if (name in ICON_PATH_CACHE) {
		return ICON_PATH_CACHE[name];
	}
	const key = `mdi${name.replace(/Icon$/, "")}`;
	const path = (mdi as Record<string, string>)[key] as string | undefined;
	ICON_PATH_CACHE[name] = path;
	return path;
}
...
export function joinUnitSvg(color = INDIGO_600, path = MDI_ACCOUNT_MULTIPLE, px = 72): string {
	return tileSvg(color, path, 1, px);
}

export function loginExpiredSvg(px = 72): string {
	return tileSvg(ERROR_600, MDI_ACCOUNT_ALERT, 1, px);
}

export function charSelectSvg(px = 72, locked = false): string {
	const rx = Math.round(px * (14 / 72));
	const badge = locked
		? `<g transform="translate(57,57) scale(0.85) translate(-12,-12)"><path d="${MDI_LOCK}" fill="#fbbf24"/></g>`
		: "";
	return (
		`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72" width="${px}" height="${px}">` +
		`<rect x="0" y="0" width="72" height="72" rx="${rx}" fill="${INDIGO_600}"/>` +
		`<g transform="translate(36,36) scale(1.9) translate(-12,-12)">` +
		`<path d="${MDI_ACCOUNT_SWITCH}" fill="#ffffff"/>` +
		`</g>` +
		badge +
		`</svg>`
	);
