...
export async function login(
	...
	try {
		...
		// If a character was chosen during login (e.g. `last_char_lock` active),
		// the returned char is the active one and its token is already final.
		if (loginResp.char?.token) {
			...
			return {
				...
				chars,
				activeChar: loginResp.char.char,
			};
		}
	} catch (e) {
