...
import { loginExpiredSvg, statusTitle, svgDataUri } from "./icons";
...
export async function sendSessionExpired(action: KeyAction): Promise<void> {
	await sendTitle(action, statusTitle("Login abgelaufen"));
	await sendImage(action, svgDataUri(loginExpiredSvg()));
}